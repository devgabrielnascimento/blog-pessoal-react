var p = Object.defineProperty, e = Object.defineProperties;
var f = Object.getOwnPropertyDescriptors;
var m = Object.getOwnPropertySymbols;
var x = Object.prototype.hasOwnProperty, h = Object.prototype.propertyIsEnumerable;
var t = (o, r, s) => r in o ? p(o, r, { enumerable: !0, configurable: !0, writable: !0, value: s }) : o[r] = s, a = (o, r) => {
  for (var s in r || (r = {}))
    x.call(r, s) && t(o, s, r[s]);
  if (m)
    for (var s of m(r))
      h.call(r, s) && t(o, s, r[s]);
  return o;
}, i = (o, r) => e(o, f(r));
import { j } from "../node_modules/.pnpm/react@18.2.0/node_modules/react/jsx-runtime.mjs";
import { forwardRef as n } from "react";
import c from "../lib/IconBase.mjs";
import d from "../defs/Trash.mjs";
const w = n((o, r) => /* @__PURE__ */ j.jsx(c, i(a({ ref: r }, o), { weights: d })));
w.displayName = "Trash";
export {
  w as Trash
};
