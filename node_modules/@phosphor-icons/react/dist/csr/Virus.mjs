var a = Object.defineProperty, e = Object.defineProperties;
var f = Object.getOwnPropertyDescriptors;
var m = Object.getOwnPropertySymbols;
var x = Object.prototype.hasOwnProperty, j = Object.prototype.propertyIsEnumerable;
var i = (o, r, s) => r in o ? a(o, r, { enumerable: !0, configurable: !0, writable: !0, value: s }) : o[r] = s, t = (o, r) => {
  for (var s in r || (r = {}))
    x.call(r, s) && i(o, s, r[s]);
  if (m)
    for (var s of m(r))
      j.call(r, s) && i(o, s, r[s]);
  return o;
}, p = (o, r) => e(o, f(r));
import { j as n } from "../node_modules/.pnpm/react@18.2.0/node_modules/react/jsx-runtime.mjs";
import { forwardRef as u } from "react";
import c from "../lib/IconBase.mjs";
import d from "../defs/Virus.mjs";
const w = u((o, r) => /* @__PURE__ */ n.jsx(c, p(t({ ref: r }, o), { weights: d })));
w.displayName = "Virus";
export {
  w as Virus
};
