var n = Object.defineProperty, p = Object.defineProperties;
var a = Object.getOwnPropertyDescriptors;
var s = Object.getOwnPropertySymbols;
var f = Object.prototype.hasOwnProperty, x = Object.prototype.propertyIsEnumerable;
var t = (r, o, m) => o in r ? n(r, o, { enumerable: !0, configurable: !0, writable: !0, value: m }) : r[o] = m, e = (r, o) => {
  for (var m in o || (o = {}))
    f.call(o, m) && t(r, m, o[m]);
  if (s)
    for (var m of s(o))
      x.call(o, m) && t(r, m, o[m]);
  return r;
}, i = (r, o) => p(r, a(o));
import { j } from "../node_modules/.pnpm/react@18.2.0/node_modules/react/jsx-runtime.mjs";
import { forwardRef as l } from "react";
import u from "../lib/IconBase.mjs";
import c from "../defs/Funnel.mjs";
const d = l((r, o) => /* @__PURE__ */ j.jsx(u, i(e({ ref: o }, r), { weights: c })));
d.displayName = "Funnel";
export {
  d as Funnel
};
