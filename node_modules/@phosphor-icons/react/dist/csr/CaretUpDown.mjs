var e = Object.defineProperty, i = Object.defineProperties;
var f = Object.getOwnPropertyDescriptors;
var m = Object.getOwnPropertySymbols;
var n = Object.prototype.hasOwnProperty, w = Object.prototype.propertyIsEnumerable;
var p = (r, o, t) => o in r ? e(r, o, { enumerable: !0, configurable: !0, writable: !0, value: t }) : r[o] = t, s = (r, o) => {
  for (var t in o || (o = {}))
    n.call(o, t) && p(r, t, o[t]);
  if (m)
    for (var t of m(o))
      w.call(o, t) && p(r, t, o[t]);
  return r;
}, a = (r, o) => i(r, f(o));
import { j as x } from "../node_modules/.pnpm/react@18.2.0/node_modules/react/jsx-runtime.mjs";
import { forwardRef as j } from "react";
import c from "../lib/IconBase.mjs";
import d from "../defs/CaretUpDown.mjs";
const C = j((r, o) => /* @__PURE__ */ x.jsx(c, a(s({ ref: o }, r), { weights: d })));
C.displayName = "CaretUpDown";
export {
  C as CaretUpDown
};
